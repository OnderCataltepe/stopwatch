...
const Table = ()=>{
    ...
    useEffect(()=>{
        if(oTimes.length>1){
            let convLapTime = convertTimeFunc(oTimes[oTimes.length-1] -oTimes[oTimes.length-2]);
            ...
            setExtremum((prev)=>[...prev, oTimes[oTimes.length-1] - oTimes[oTimes.length-2]]);
        };  
    },[oTimes.length])
            ...
            <table>
                ...
                <tbody>
                    {timeList.length>0 && timeList.map((time,index)=>{
                        return <tr key={index}>
                                    <td className={styles[classLap(time.index)]}>{time.index}</td>
                                    <td>{time.lapTime.hours}:{time.lapTime.minutes}:{time.lapTime.seconds}.{time.lapTime.mSeconds}</td>
                                    <td>{time.ovTime.hours}:{time.ovTime.minutes}:{time.ovTime.seconds}.{time.ovTime.mSeconds}</td>
                               </tr>
                    })
                    }
                </tbody>
